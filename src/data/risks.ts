import type { Risk } from '../types/risk';

// Only accounts with an explicitly stated risk/concern/dependency in PROJECT_CONTEXT.md
// have a Risk record. Manprec has no "Primary concern" or comparable line in the source
// document, so no Risk was fabricated for it — see the Phase 1 report.
export const risks: Risk[] = [
  {
    id: 'risk-siemens-regulatory-doc-approval',
    customerId: 'siemens',
    title: 'Regulatory Documentation module pending Siemens IT approval',
    description:
      'External dependency on Siemens IT approval for the Regulatory Documentation module. Explicitly treated as low-severity and not a customer-health blocker.',
    severity: 'low',
    status: 'monitoring',
    dependencyType: 'external',
    relatedDimension: 'executionRisk',
    shortTitle: 'Documentación Regulatoria',
    shortCause: 'Pendiente de aprobación de TI de Siemens',
    dependencyTypeLabel: 'Dependencia externa',
    healthImpactStatement: 'Sin impacto actual en HealthScore',
    relatedEvidenceIds: ['ev-siemens-dependency'],
  },
  {
    id: 'risk-balle-adoption-momentum',
    customerId: 'grupo-balle',
    title: 'Dependence on implementation/support momentum',
    description:
      'Primary concern is sustaining recent workflow adoption gains and reducing reliance on implementation/support-driven momentum.',
    severity: 'medium',
    status: 'resolved',
    relatedDimension: 'workflowAdoption',
    shortTitle: 'Dependencia del momentum de implementación',
    shortCause: 'Sostener el uso reciente del flujo y avanzar hacia una adopción operativa durable.',
    relatedEvidenceIds: ['ev-balle-kickoff-delay', 'ev-balle-multivault'],
  },
  {
    id: 'risk-balle-second-vault-legal-entity',
    customerId: 'grupo-balle',
    title: 'Segundo vault pendiente de definición de entidad legal',
    description:
      'El segundo vault está bloqueado mientras Grupo Balle define qué entidad legal / razón social debe usarse.',
    severity: 'low',
    status: 'monitoring',
    openedAt: '2026-09-09',
    dependencyType: 'external',
    relatedDimension: 'executionRisk',
    shortTitle: 'Definición legal para segundo vault',
    shortCause: 'El segundo vault depende de definir la razón social correcta.',
    dependencyTypeLabel: 'Dependencia del cliente',
    healthImpactStatement: 'Dependencia operativa acotada; no implica Commercial Attention.',
    relatedEvidenceIds: ['ev-balle-second-vault-legal-entity-2026-09-09'],
  },
  {
    id: 'risk-manprec-technical-workflow-completion',
    customerId: 'manprec',
    title: 'Fricción para completar workflow técnico',
    description:
      'El workflow técnico reciente se detuvo alrededor de 2%, con fricción por procesamiento de documentos/manuales pesados y conversión .doc.',
    severity: 'medium',
    status: 'open',
    openedAt: '2026-09-09',
    relatedDimension: 'executionRisk',
    shortTitle: 'Fricción técnica de procesamiento',
    shortCause: 'Análisis detenido cerca de 2% con documentos pesados y fricción de conversión .doc.',
    healthImpactStatement: 'Requiere validar un rerun técnico end-to-end antes de considerarlo resuelto.',
    relatedEvidenceIds: [
      'ev-manprec-analysis-stalled-2026-09-09',
      'ev-manprec-heavy-docs-friction-2026-09-09',
      'ev-manprec-doc-conversion-friction-2026-09-09',
    ],
  },
  {
    id: 'risk-fibroptica-role-activation',
    customerId: 'fibroptica',
    title: 'Required-role activation / adoption breadth',
    description:
      'Only ~3 of 13 users are active. Unclear whether this reflects the correct set of required operating roles or a broader adoption-breadth risk.',
    severity: 'medium',
    status: 'open',
    relatedDimension: 'requiredRoleActivation',
    shortTitle: 'Activación de roles requeridos',
    shortCause: 'Sólo 3 de 13 usuarios están activos; falta confirmar si son los roles operativos correctos.',
    relatedEvidenceIds: ['ev-fibroptica-adoption-breadth', 'ev-fibroptica-role-question'],
  },
];
